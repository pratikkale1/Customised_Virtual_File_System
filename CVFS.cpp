...
#define MAXINODE 50 // macro use for maximum inode = 50 , it may varry as per programmer need

#define READ 1 // Permmisions
#define WRITE 2  //Permmisions ( READ + WRITE == 1 + 2 = 3)

#define MAXFILESIZE 2048 // macro use for to decide Maximum file size , it may varry as per programmer need

#define REGULAR 1 // macro generally use for to work on regular files
#define SPECIAL 2 // In future if wand to work on special file`
...
UFDT UFDTArr[50]; // Global variable 
...
void DisplayHelp()
{
    ...
    printf("fstat : To Display informtion of file using file descriptor\n");
    ...
    while(1)
    {
        ...
        else if(count == 3)
        {
            ...
            else if(strcmp(command[0],"read")== 0)
            {
                ...
                ret = ReadFile(fd,ptr,atio(command[2]));
                if(ret == -1)
                    printf("ERROR : File not existing\n");
                if (ret == -2)
                    printf("ERROR : Permmision denied\n");
                if(ret == -3)
        ...
        else if(count ==4)
        {
            if(strcmp(command[0],"lseek")==0)
            {
                ...
                if(fd == -1)
                {
                    printf("ERROR : Incorrect paratmeters\n");
                    continue;
                }
                ret = LseekFile(fd,atio(command[2],atio(command[3])));
                if(ret == -1)
                {
